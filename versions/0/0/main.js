...
import Popup, {
	sleep,
} from "https://gavinmorrow.github.io/EasyJS/2/ui/popup/index.js";
...
(async () => {
	...
	window.errorFuncs = {
		friendFetch: getErrorFunc(
			e =>
				(friendsElem.innerHTML = `There was an error. Refresh the page and try again.<br/><br/>${e}`)
		),
		addFriend: getErrorFunc(async e => {
			const popup = await new Popup(
				`There was an error. Refresh the page and try again. <br/><br/>${e}`,
				5000,
				false,
				"",
				true
		...
		friendNoExist: getErrorFunc(async (_, username) => {
			new Popup(
				`The username you inputed (${username}) does not exist.`,
				5000,
				false,
				"",
				true
	...
	const fetchFriends = async () => {
		const file = await fetch(
			`${srcURL}/file.php?username=${username}&path=/&name=friends.txt`
		)
			.then(r => r.text(), noCache)
			.catch(errorFuncs.friendFetch);
		switch (file) {
			case "0":
		...
		const friendsString = await fetch(
			`${databaseURL}/users/${username}/friends.txt`
		).then(r => r.text());
...
`,
	...
	const addFriend = async e => {
		if (e.target == document.getElementById("add-friend")) {
			await popup.show();
			document
				.getElementById("submit-friend")
				.addEventListener("click", async () => {
					document
						.getElementById("submit-friend")
						.setAttribute("disabled", "");
					const addFriend = await fetch(
						`${srcURL}/addFriend.php?username=${username}&fusername=${
						...
						.then(r => r.text())
						.catch(errorFuncs.addFriend);
					switch (addFriend) {
						case "0":
							errorFuncs.addFriend(
								new Error("The username does not exist."),
								document.getElementById("friend-username").value
							);
							break;
						case "1":
	...
	document
		.getElementById("enter-brawlosseum")
		.addEventListener("click", async () => {
