...
const main = () => {
	...
	play.addEventListener("click", async () => {
		...
		// Get the HTML
		const gameHTML = await fetch(
			`/versions/${majorVersion}/${minorVersion}/main.html`
		)
			.then(r => r.text())
			.then(
				html =>
					html.split("<!-- GAME HTML -->")[1] ||
					`<!-- GAME HTML -->There was an error. Reload the page and try again.\n\n${e}<!-- GAME HTML -->`
			)
			.catch(e => {
				alert(
				...
				console.error(e);
				return `<!-- GAME HTML -->There was an error. Reload the page and try again.\n\n${e}<!-- GAME HTML -->`;
			});
...
(async () => {
	if (await ili()) main();
	// User is logged in, so let them into page
